...
tax_df <- ImportTable(file = '../02.Filter_samples//2021-08-03-Selected-Bac_RawData-Species-v1.0.csv', sep = ',')
...
red_1329 <- colnames(ImportTable(file = '../05.Normalized/Rarefy_1329_fungi/2021-07-26-RawAbundance_matrix-v1.0.csv'))

reduce_tax_df <- tax_df[, red_1329]
reduce_meta_df <- meta_df[red_1329, ]
...
if (calculate_or_not) {
  ...
  alpha_rds <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/AlphaIndex/', 
  ...
  for (sub_set in names(alpha_res)) {
    sub_alpha <- alpha_res[[sub_set]] %>% 
                 ...
                 as.data.frame()
    sub_alpha_csv <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/AlphaIndex/table', 
  ...
  for (alphIdx in colnames(sub_alpha)[c(1,5,6)]) {
    ...
    pdf_file <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/AlphaIndex/figure', 
  ...
  gplot_list_rds <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/AlphaIndex/',
                               Prefix = 'gplot_list', Suffix = 'rds')
  saveRDS(object = gplot_list, file = gplot_list_rds)
}else{
  alpha_res <- readRDS(file = '../09.Bacteria/06.AlphaDiversity/AlphaIndex/2021-07-27-alpha_res-v1.0.rds')
  gplot_list <- readRDS(file = '../09.Bacteria/06.AlphaDiversity/AlphaIndex/2021-07-27-gplot_list-v1.0.rds')
}
...
# select the bac phylum
sel_bac_phy <- unique(tax_name_df$Phylum[grep('Bacteria', tax_name_df$Kingdom)]) 
sel_bac_phy <- sel_bac_phy[! is.na(sel_bac_phy)]
# select the samples and bac phylum
fungi_phy_df <- raw_phylum_df[sel_bac_phy, rownames(reduce_meta_df)]
# calculate the bacteria phylum level relative abundance 
fu_phy_rel_df <- sweep(fungi_phy_df, 2, colSums(fungi_phy_df), "/")*100
mean_fu_phy_ <-  sort(rowMeans(fu_phy_rel_df), decreasing = T)
Show_table(data.frame(MeanAbundance =mean_fu_phy_))
```
...
```{r}
calculate_or_not <- F
mean_fu_phy <- mean_fu_phy_
sel_phy_list <- mean_fu_phy[mean_fu_phy > 0.1]
...
if (calculate_or_not) {
  ...
  for (i in unique(sel_phy_df$Cohort)) {
    ...
    pre_gplot <- ggplot(test_melt, aes(x = factor(SampleName, levels = pre_cohort_df$SampleName),
      ...
      ggtitle(i)
    pre_gplot_pdf <- FileCreate(DirPath = paste0('../09.Bacteria/06.AlphaDiversity/Fungal_Components/Top2/', i),
    ...
    plot(pre_gplot)
    dev.off()
    pre_table_csv <- FileCreate(DirPath = paste0('../09.Bacteria/06.AlphaDiversity/Fungal_Components/Top2/', i, '/pre-table'),
                                Prefix = paste0('Prediction_Table-', i,'-top2'), Suffix = 'csv')
    write.csv(x = pre_cohort_df, file = pre_table_csv)
    
    raw_table_csv <- FileCreate(DirPath = paste0('../09.Bacteria/06.AlphaDiversity/Fungal_Components/Top2/', i, '/raw-table'),
  ...
  test_melt$Stage <- factor(ifelse(test_melt$Stage == 'adenoma',
    ...
    pre_gplot <- ggplot(test_melt, aes(x = factor(SampleName, levels = pre_cohort_df$SampleName),
      ...
      theme(axis.text = element_blank(),
            ...
            axis.ticks =element_blank())+
      scale_fill_ochre(option = 'olsen_qual')
  pre_gplot_pdf <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/Fungal_Components/Top2/all',
                              Prefix = 'Components_bar-all-top2', Suffix = 'pdf')
  pdf(pre_gplot_pdf, width = 20, height = 5)
  plot(pre_gplot + theme(legend.position = 'none'))
  plot(pre_gplot)
  dev.off()
  pre_table_csv <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/Fungal_Components/Top2/all/pre-table',
                              Prefix = paste0('Prediction_Table-all-top2'), Suffix = 'csv')
  write.csv(x = pre_cohort_df, file = pre_table_csv)
  
  raw_table_csv <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/Fungal_Components/Top2/all/raw-table',
                              Prefix = paste0('Raw_Splited_Table-all-top2'), Suffix = 'csv')
  write.csv(x = sub_cohort_df, file = raw_table_csv)
  
  
  
  phy_df_rds <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/Fungal_Components', 
                           Prefix = 'phy_df_list', Suffix = 'rds')
  saveRDS(object = phy_df_list, file = phy_df_rds)
  
  pre_phy_rds <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/Fungal_Components', 
...
sel_feature <- names(sel_phy_list)
...
boxplot_pdf <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/Normaled_Median',
...
melt_median_df$ALL <- rep("ALL", nrow(melt_median_df))
box_plot_all <- ggplot(melt_median_df, aes(x = ALL, y = `Relative Median Abudance`))+
  geom_violin(aes(fill = Stage),  scale = "width",trim = F,
              position=position_dodge()) +
  geom_boxplot(aes(color = Stage), outlier.color = NA,
               width=0.2, fill="white", position = position_dodge(width =0.9))+
...
boxplot_pdf_all <- FileCreate(DirPath = '../09.Bacteria/06.AlphaDiversity/Normaled_Median',
                              Prefix = "boxplot-normalized_control_median-phylum-all",
                              Suffix = 'pdf')
pdf(file = boxplot_pdf_all, width = 4, height = 15)
plot(box_plot_all)
plot(box_plot_all + scale_y_log10())
dev.off()
