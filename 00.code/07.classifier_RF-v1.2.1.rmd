...
```{r , message = F, warning = F}
meta_df <- ImportTable(file = '../00.RawData/metaData/2022-04-11-metaInfo-subgroup_1329-v7.0.csv')
...
for (c in cohort_lst) {
  ...
  if (all(is.na(cn_meta_df[cn_meta_df$Cohort == c,"AJCC_simple"]))) {
    next
  }
  sub_type = 'all'
  Importance_df2 <- ImportTable(file = paste0('../11.Classifier/RF/FeatureSel_Important/',
  ...
  early_list <- rownames(cn_meta_df)[(cn_meta_df$Cohort == c &cn_meta_df[,"AJCC_simple"] %in% c("I", "II")) | (cn_meta_df$Cohort == c & cn_meta_df[,"Stage"] == "CTRL")]
  valda_set_early <- sel_imp_df[early_list, ]
  
  later_list <- rownames(cn_meta_df)[(cn_meta_df$Cohort == c &cn_meta_df[,"AJCC_simple"] %in% c("III", "IV")) | (cn_meta_df$Cohort == c & cn_meta_df[,"Stage"] == "CTRL")]
  valda_set_later <- sel_imp_df[later_list, ]
  if (sum(cn_meta_df$Cohort == c &cn_meta_df[,"AJCC_simple"] %in% c("I", "II")) < 10 | 
      sum(cn_meta_df$Cohort == c &cn_meta_df[,"AJCC_simple"] %in% c("III", "IV")) < 10 ) {
    next
  }
  set.seed(best_seed_list[[sub_type]])
  # set.seed(123)
  ...
  pred_result <- prediction(pred, valda_set[,1])
  auc_value <- round(performance(pred_result, measure = "auc")@y.values[[1]]*100, 2)
  all_roc <- roc(valda_set[,1], pred, legacy.axes = T, percent = T, ci=TRUE)
  
  # early crc
  pred_early <- predict(rf, newdata=valda_set_early[-1], type = 'prob')[,2]
  pred_result_early <- prediction(pred_early, valda_set_early[,1])
  auc_value_early <- round(performance(pred_result_early, measure = "auc")@y.values[[1]]*100, 2)
  all_roc_early <- roc(valda_set_early[,1], pred_early, legacy.axes = T, percent = T)
  
  # later crc
  pred_later <- predict(rf, newdata=valda_set_later[-1], type = 'prob')[,2]
  pred_result_later <- prediction(pred_later, valda_set_later[,1])
  auc_value_later <- round(performance(pred_result_later, measure = "auc")@y.values[[1]]*100, 2)
  all_roc_later <- roc(valda_set_later[,1], pred_later, legacy.axes = T, percent = T)
  
  # plot 
  roc_pdf <- FileCreate(DirPath = "../11.Classifier/RF/Comparison_stage/", Prefix = paste0('ROC-', c), Suffix = 'pdf')
  pdf(file = roc_pdf)
  par(pty = "s")
  plot(all_roc, col = rgb(76, 174, 204, 100, maxColorValue=255), lwd = 4, legacy.axes = T, lty = 1,
       xlab = "False Postive Percentage", ylab = "True Postive Percentage", cex = 1.3)
  # lines(roc_list$fungi, col = '#ff420e', lwd = 2, lty = 1)
  lines(all_roc_early, col = rgb(251,202,80, 150, maxColorValue=255), lwd = 2, lty = 5)
  lines(all_roc_later, col = rgb(251, 0, 7, 150, maxColorValue=255), lwd = 2, lty = 6)
  text(x = 0, y = 25, labels =paste0("CRC AUC = ", auc_value, "%"), adj = 1, col = rgb(76, 174, 204, 255, maxColorValue=255), cex = 1.8)
  text(x = 0, y = 15, labels =paste0("Late CRC AUC = ", auc_value_later, "%"), adj = 1, col = rgb(251, 0, 7, 255, maxColorValue=255), cex = 1.8)
  text(x = 0, y = 5, labels =paste0("Early CRC AUC = ", auc_value_early, "%"), adj = 1, col = rgb(251,202,80, 255, maxColorValue=255), cex = 1.8)
  dev.off()
