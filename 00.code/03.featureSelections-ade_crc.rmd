...
if (calculate_or_not) {
  ...
  data1_m <- core_modify_Comb[rownames(meta_df)[meta_df$Stage == 'CRC'], ] %>%
    t() %>% as.data.frame()
  data2_m <- core_modify_Comb[rownames(meta_df)[meta_df$Stage == 'adenoma'], ] %>%
    t() %>% as.data.frame()
  ...
  # wilcoxon-rank test 
  mData <- as.data.frame(t(core_modify_Comb[rownames(meta_df)[meta_df$Stage %in% c('adenoma', 'CRC')],]))
  ...
  wil_res <- ImportTable('../09.euk-ade_crc/2021-10-07-wilcoxon-all-v1.0.0.csv')
}
