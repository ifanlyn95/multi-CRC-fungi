...
gg_wil_data$name <- factor(rownames(gg_wil_data), rownames(gg_wil_data))
...
g_bar <- ggplot(data = gg_wil_data, mapping = aes(x = name, y = `-log_q`))+
  geom_bar(stat="identity", fill = ifelse(gg_wil_data$`-log_q` > 2, "snow4", 'snow3'), 
...
pdf(file = barplot_pdf, width = 15, height = 7)
plot(g_bar)
dev.off()
