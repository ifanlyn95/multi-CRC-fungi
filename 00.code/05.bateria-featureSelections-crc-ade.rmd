...
if (calculate_or_not) {
  ...
  ctrl_modify_Comb <- modify_Comb[rownames(meta_df)[meta_df$Stage == 'adenoma'], ]
  crc_modify_Comb <- modify_Comb[rownames(meta_df)[meta_df$Stage == 'CRC'], ]
  
  #  mean abundance > 0.01%

  
  comb_cohort <- read.csv("../09.Bacteria/02.RelMedianAbundance/2021-08-05-normalized-0.1%-RelMedianAbundance-matrix-v1.0.0.csv", 
  ...
  wil_csv <- FileCreate(DirPath = '../15.Revision/09.Bacteria-CRC-ade/', Prefix = paste0('wilcoxon-', 'all'), Suffix = 'csv')
  ...
  wil_res <- ImportTable('../15.Revision/09.Bacteria-CRC-ade/2022-04-07-wilcoxon-all-v1.0.0.csv')
}
...
volcano_plot  <- FileCreate(DirPath = '../15.Revision/09.Bacteria-CRC-ade/', Prefix = paste0('volcanoPlot-', 'all'), Suffix = 'pdf')
