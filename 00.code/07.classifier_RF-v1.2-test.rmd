...
# for (sub_type in c('bac', 'all')) {
  ...
  # sel_imp_df <- as.data.frame(cbind(sel_imp_df,cn_meta_df[rownames(sel_imp_df), c("Age", "Gender")]))
  
  # auc_df <- as.data.frame(matrix(data = NA, nrow = 100, ncol = 9))
  auc_df <- as.data.frame(matrix(data = NA, nrow = 100, ncol = 2))
  rownames(auc_df) <- paste0("Seed_", 1:100)
  # colnames(auc_df) <- c(cohort_lst, "Mean")
  colnames(auc_df) <- c("2014_ZellerG", "Mean")
  auc_mean <- 0
  auc_seed <- 1
  for (k in c(1:100)) {
    auc_tmp <- NULL
    for (c in '2014_ZellerG') {
      train_set <- na.omit(sel_imp_df[rownames(cn_meta_df)[cn_meta_df$Cohort != c], ]) 
      valda_set <- na.omit(sel_imp_df[rownames(cn_meta_df)[cn_meta_df$Cohort == c], ])
      set.seed(k)
      # set.seed(123)
...
for (c in cohort_lst) {
  ...
  auc_tmp <- 0
  cand_tmp <- NA
  for (sel_cand in sel_all_list) {
    glm.fit <- glm(as.formula(paste0("Stage ~ ", sel_cand)), train_set, family = binomial)
    roc.res <- roc(train_set$Stage, glm.fit$fitted.values, quiet = T)
    auc_value <- roc.res$auc
    if (auc_tmp < auc_value) {
      auc_tmp <- auc_value
      cand_tmp <- sel_cand
    }
  }
  message(c, "\t", cand_tmp, "\t", auc_tmp)
